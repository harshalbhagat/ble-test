...
namespace BTLE.Misc
    {
    public class LemondVibrationPatternSlot
        {
        public LemondVibrationPatternSlot( VibrationPatternSpecifier specifier, uint value )
            {
            Specifier = specifier;
            Value = value;
            }

        public static byte CorrectBitFromDelayIn10MilliSec( int delayIn10MilliSec )
            {
            // Based on our LRA, a delay is encoded with bit 7 set
            // and bit [6:0] * 10 ms as wait time
            //

            return (byte)( delayIn10MilliSec | ( 0x1 << 7 ) );
            }

        public VibrationPatternSpecifier Specifier
            {
            get;
            }
        ...
        private static string [] _waveforms =
            {
                @"0 None",
                @"1 Strong Click - 100%",
                @"2 Strong Click - 60%",
        ...
        public override string ToString()
            ...
            description.AppendFormat( "Specifier = {0},\n", Specifier );
            description.AppendFormat( "Value = {0},\n", Value );
